...
def exact(game, batch_size=None):
    '''Calculate feature attributions using the Shapley value.'''
    N = 0
    mean = 0
    sum_squares = 0
    ...
    # Determine batch size.
    if batch_size is None:
        batch_size = np.math.factorial(game.players)
    arange = np.arange(batch_size)
    ...
    l = list(permutations(range(game.players)))
    ...
    for i in range(game.players):
        S[arange, permus[:, i]] = 1
        next_value = game(S)
        deltas[arange, permus[:, i]] = next_value - prev_value
        prev_value = next_value
